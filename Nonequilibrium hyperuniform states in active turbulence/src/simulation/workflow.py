...
def run(parameters: dict) -> str:
    """
    Run a complete active flow simulation.
    
    Executes the full simulation pipeline:
    1. Initialize Neptune.ai run for experiment tracking
    2. Set up computational grids and operators
    3. Configure the PVC model (effective viscosity)
    4. Run time-stepping loop
    5. Save results and upload to Neptune
    ...
    -------
    str
        Neptune.ai run ID for this experiment (can be used in subsequent analyses).
    
    Notes
    -----
    Results are automatically uploaded to Neptune.ai and local files are
    deleted after upload to enforce cloud-first data management.
    """
